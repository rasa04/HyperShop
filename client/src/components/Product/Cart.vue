<template>
  <section id="cart_products" v-if="this.cart.length > 0">
        ...
        <tr v-for="product in this.products" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
          ...
          <td class="px-6 py-4">
            <div class="flex items-center space-x-3">
              <button class="inline-flex items-center p-1 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-full
              ...
              <div>
                ...
                focus:ring-blue-500 focus:border-blue-500 block px-2.5 py-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white
                dark:focus:ring-blue-500 dark:focus:border-blue-500" :placeholder="product.amount" min="1" :max="product.quantity" required>
              </div>
              <button class="inline-flex items-center p-1 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-full
              focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600
              dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700" type="button">
          ...
          <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white">{{ product.quantity }}</td>
          <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white">{{ product.price }}</td>
          <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white">{{ product.price }}</td>
          <td class="px-6 py-4">
            <button @click.prevent="removeFromCard(product.id)"
                    class="font-medium text-red-600 dark:text-red-500 hover:text-white hover:bg-red-500 p-1 rounded-lg transition duration-300 ease-out"
            >Remove</button>
          </td>
        </tr>
        </tbody>
  ...
  <section id="do_action" class="w-full text-center flex flex-col items-center" v-if="this.cart.length > 0">
    <div class="w-full font-bold select-none bg-lime-100 my-2">
      <h3>What would you like to do next?</h3>
      <p>Choose if you have a discount code or reward points you want to use or would like to estimate your delivery cost.</p>
    </div>
    <div class="w-full grid grid-cols-2 gap-2 select-none">
      <div class="uppercase grid place-items-center bg-lime-100 rounded-lg">
        <h3 class="mb-5 text-lg font-medium text-gray-900 dark:text-white">Specify order:</h3>
        <ul class="grid w-full gap-6 md:grid-cols-3">
          <li>
            <input type="checkbox" id="react-option" value="" class="hidden peer" required="">
            <label for="react-option" class="inline-flex items-center justify-between w-full p-5 text-gray-500 bg-white border-2 border-gray-200 rounded-lg cursor-pointer
              dark:hover:text-gray-300 dark:border-gray-700 peer-checked:border-blue-600 hover:text-gray-600 dark:peer-checked:text-gray-300 peer-checked:text-gray-600
              hover:bg-gray-50 dark:text-gray-400 dark:bg-gray-800 dark:hover:bg-gray-700">
              <div class="w-full text-lg font-semibold">Coupon</div>
            </label>
          </li>
          <li>
            <input type="checkbox" id="flowbite-option" value="" class="hidden peer">
            <label for="flowbite-option" class="inline-flex items-center justify-between w-full p-5 text-gray-500 bg-white border-2 border-gray-200 rounded-lg cursor-pointer
              dark:hover:text-gray-300 dark:border-gray-700 peer-checked:border-blue-600 hover:text-gray-600 dark:peer-checked:text-gray-300 peer-checked:text-gray-600
              hover:bg-gray-50 dark:text-gray-400 dark:bg-gray-800 dark:hover:bg-gray-700">
              <div class="w-full text-lg font-semibold">Voucher</div>
            </label>
          </li>
          <li>
            <input type="checkbox" id="angular-option" value="" class="hidden peer">
            <label for="angular-option" class="inline-flex items-center justify-between w-full p-5 text-gray-500 bg-white border-2 border-gray-200 rounded-lg cursor-pointer
              dark:hover:text-gray-300 dark:border-gray-700 peer-checked:border-blue-600 hover:text-gray-600 dark:peer-checked:text-gray-300 peer-checked:text-gray-600
              hover:bg-gray-50 dark:text-gray-400 dark:bg-gray-800 dark:hover:bg-gray-700">
              <div class="w-full text-lg font-semibold">Gift Card</div>
            </label>
          </li>
        </ul>

        <ul id="location" class="">
          <div class="flex p-2">
            <button id="states-button" data-dropdown-toggle="dropdown-states" class="flex-shrink-0 z-10 inline-flex items-center py-2.5 px-4 text-sm font-medium text-center
            text-gray-500 bg-gray-100 border border-gray-300 rounded-l-lg hover:bg-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 dark:bg-gray-700
            dark:hover:bg-gray-600 dark:focus:ring-gray-700 dark:text-white dark:border-gray-600" type="button">
              <svg aria-hidden="true" class="h-3 mr-2" viewBox="0 0 15 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <g mask="url(#mask0_12694_49953)">
                  <path fill-rule="evenodd" clip-rule="evenodd" d="M14.5 0H0.5V0.8H14.5V0ZM14.5 1.6H0.5V2.4H14.5V1.6ZM0.5 3.2H14.5V4H0.5V3.2ZM14.5 4.8H0.5V5.6H14.5V4.8ZM0.5 6.4H14.5V7.2H0.5V6.4ZM14.5 8H0.5V8.8H14.5V8ZM0.5 9.6H14.5V10.4H0.5V9.6ZM14.5 11.2H0.5V12H14.5V11.2Z"
                        fill="#D02F44"/>
        ...
        <div class="w-full grid grid-cols-2 place-items-center">
          <button class="p-2" href="">Get Quotes</button>
          <RouterLink :to="{name: 'checkout'}" class="btn btn-default check_out" href="">Continue</RouterLink>
        </div>
      </div>
      <div class="font-bold uppercase grid place-items-center bg-lime-100 rounded-lg">
        <ul class="">
          <li>Cart Sub Total <span>{{ this.totalPrice }}</span></li>
          <li>Eco Tax <span>$2</span></li>
          <li>Shipping Cost <span>Free</span></li>
          <li>Total <span>{{ this.totalPrice }}</span></li>
        </ul>
        <RouterLink :to="{name: 'checkout'}" class="bg-lime-700 p-2 rounded-lg hover:bg-lime-600 ease-in duration-300">Check Out</RouterLink>
      </div>
    </div>
  </section>
  <section class="w-full text-5xl text-bold text-center text-gray-500 p-10 select-none" v-if="this.cart === 0">
    NO ORDERS
  </section>
</template>
...
export default {
  ...
  methods: {
      ...
      axios.get(`${this.VITE_API_URL}/api/products`, { params: {'id': temp}})
        ...
        this.products.forEach(product => {
          this.cart.forEach(cart_product => {
            if (cart_product.id === product.id) product['amount'] = cart_product.quantity
          })
        })
    ...
    removeFromCard(id) {
      localStorage.setItem('cart', JSON.stringify(this.cart.filter(productInCart => productInCart.id !== id)))
      this.getCart()
      console.log('works')
    }
  },
  ...
  computed: {
    totalPrice() {
      ...
      Object.values(this.products).forEach(product => { totalPrice += product.price})
      return totalPrice
    }
  },
